...
class Ader(commands.Bot):
    ...
    async def on_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        """Always acknowledge failed slash commands instead of leaving 'This application did not respond'."""
        ...
        if isinstance(original,app_commands.CheckFailure):
            ...
            if getattr(original,'code',None)==50001: message='❌ البوت ما عندوش Access لهاد السيرفر/الروم.'
            else: message='❌ البوت ما عندوش الصلاحيات الكافية لتنفيذ هاد الأمر.'
        elif isinstance(original,discord.HTTPException) and getattr(original,'code',None)==350005:
            message='❌ Discord رفض العملية بسبب إعدادات Server Onboarding. خاص يبقى على الأقل روم واحد @everyone يقدر يقرا فيه ويرسل الرسائل.'
        else:
