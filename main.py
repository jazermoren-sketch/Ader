...
class Ader(commands.Bot):
    def __init__(self, config: dict):
        ...
        self.config=config; self.start_time=discord.utils.utcnow(); self.logger=BotLogger(config.get('logging',{}))
    ...
    async def on_ready(self):
        ...
        typ=self.config.get('bot',{}).get('activity_type','watching'); text=self.config.get('bot',{}).get('activity','your community')
        await self.change_presence(activity=discord.Activity(type=types.get(typ,discord.ActivityType.watching),name=text))
        try: self.logger.info(f'Synced {len(await self.tree.sync())} application commands')
    ...
    async def on_command_error(self,ctx,error):
        ...
        if isinstance(error,commands.MissingPermissions): await ctx.send('❌ You do not have permission to use this command.',delete_after=6); return
        if isinstance(error,commands.MissingRequiredArgument): await ctx.send(f'❌ Missing argument: `{error.param.name}`',delete_after=6); return
