...
class OfficialShortcuts(commands.Cog):
    ...
    async def _owner_give(self, message: discord.Message) -> None:
        if message.author.id != BOT_OWNER_ID:
            await message.channel.send("❌ هاد الاختصار مخصص لصاحب البوت فقط.", delete_after=6)
            return

        parts = message.content.split()
        if len(parts) < 3 or not message.mentions:
            await message.channel.send("❌ الاستعمال: `!اعطي @user المبلغ`", delete_after=7)
            return

        try:
            amount = int(parts[-1].replace(",", ""))
        except ValueError:
            await message.channel.send("❌ المبلغ خاصو يكون رقم صحيح.", delete_after=6)
            return

        if amount <= 0:
            await message.channel.send("❌ المبلغ خاصو يكون أكبر من 0.", delete_after=6)
            return

        target = message.mentions[0]
        if target.bot:
            await message.channel.send("❌ ما يمكنش تعطي العملة لبوت.", delete_after=6)
            return
    ...
    async def on_message(self, message: discord.Message):
        ...
        if content.lower() in self.FIXED_BALANCE_ALIASES:
