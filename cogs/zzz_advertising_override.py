...
class FreeAdSetupView(discord.ui.View):
    def __init__(self, cog, actor_id: int, owner_id: int):
        ...
        for label, style, mention in [
            ("Everyone", discord.ButtonStyle.danger, "everyone"),
            ("Here", discord.ButtonStyle.success, "here"),
        ]:
    ...
    async def select(self, interaction: discord.Interaction, mention: str):
        ...
        if row:
            ...
            if channel is None:
                return await interaction.response.send_message(
                    "❌ تعذر إنشاء روم الإعلان. تأكد من أن البوت يملك صلاحية **Manage Channels**.",
                    ephemeral=True,
                )
        ...
        await interaction.response.send_modal(
            self.cog.AdModal(self.cog, self.owner_id, channel_id, mention, self.actor_id)
        )


class AdvertisingCommandOverride(commands.Cog):
    """Canonical $اعلان implementation.

    It deliberately replaces the older command implementation so one message
    can never execute two advertisement handlers. It also provisions an ad
    room when the target did not buy an advertising-room product.
    """
    ...
    async def cog_load(self):
        ...
        if self.ad_cog is None:
            return
        # Remove the old prefix command before registering the canonical one.
    ...
    async def create_command_ad_room(self, guild: discord.Guild, owner_id: int):
        ...
        try:
            channel = await guild.create_text_channel(
                self.ad_cog.clean_name(f"ad-{member.display_name}"),
                category=None,
    ...
    async def advertise(self, ctx: commands.Context, member: discord.Member | None = None):
        # Hard idempotency guard: the same Discord message is processed once.
        ...
        try:
            ...
            if not await self.ad_cog.authorized(ctx.author):
                return await ctx.reply(
                    "❌ هذا الأمر محمي. يلزم Administrator أو رتبة إعلان مسموح بها.",
                    mention_author=False,
                )

            # Buying the Shop product is NOT required for this administrative command.
            row = await self.db.fetchone(
                "SELECT * FROM ad_rooms WHERE guild_id=? AND owner_id=? AND active=1",
                (ctx.guild.id, member.id),
            )
            if row is None:
                channel = await self.create_command_ad_room(ctx.guild, member.id)
                if channel is None:
                    return await ctx.reply(
                        "❌ تعذر إنشاء روم الإعلان. تأكد من صلاحية **Manage Channels** للبوت.",
                        mention_author=False,
                    )

            row = await self.db.fetchone(
                "SELECT channel_id FROM ad_rooms WHERE guild_id=? AND owner_id=? AND active=1",
                (ctx.guild.id, member.id),
            )
            if row is None:
                return await ctx.reply("❌ تعذر تجهيز روم الإعلان.", mention_author=False)
